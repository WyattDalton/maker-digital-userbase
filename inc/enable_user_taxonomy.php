...
if (!class_exists('UB_User_Taxonomies')) {

class UB_User_Taxonomies {
    
private static $instance;

private static $taxonomies	= array();
var $namespace = 'usrbse';
...
static function write_log($log) {
	 ...
	 * They will behave in the same way as post taxonomies under the Posts menu item
	...
	public function user_profile($user) {
		...
		foreach(self::$taxonomies as $key=>$taxonomy){
			...
			$input_type = ( isset($taxonomy->single_value) && ($taxonomy->single_value == true) ) ? 'radio' : 'radio' ;
			?>
...
<input type="hidden" id="<?php echo $this->namespace.'-'.$key.'-backend_edit'; ?>" name="<?php echo $this->namespace.'-'.$key.'-backend_edit'; ?>" value="1" />
	 ...
	 * show_admin_column
	 */
	public function ub_user_taxonomies_add_user_id_column($columns) {
$args=array(
...
$columns[$taxonomy->name] = $taxonomy->labels->name;
	 ...
	 */
	private function ub_user_taxonomies_get_user_taxonomies($user, $taxonomy, $page = null) {
...
private function set_terms_for_user( $user_id, $taxonomy, $terms = array(), $bulk = false ) {
	 ...
	 * 
	 */
	public function ub_user_taxonomies_add_taxonomy_column_content($value, $column_name, $user_id) {
if (taxonomy_exists($column_name)) {
return $this->ub_user_taxonomies_get_user_taxonomies($user_id,$column_name);
} else {
    return $value;
}
}
...
<input id="<?php echo $this->namespace;  ?>-<?php echo $taxonomy->name;  ?>-bulk_edit-taxonomy" name="<?php  echo $this->namespace;  ?>-bulk_edit-taxonomy" value="<?php echo $taxonomy->name; ?>" type="hidden" />
	...
	public function __construct() {
		...
		add_filter('manage_users_columns', array($this, 'ub_user_taxonomies_add_user_id_column'));
		add_action('manage_users_custom_column', array($this, 'ub_user_taxonomies_add_taxonomy_column_content'), 10, 3);
...
$ub_user_taxonomies_instance = UB_User_Taxonomies::get_instance();
